...
func (dailyWordJob *DailyWordJob) Run() {
	...
	if err != nil {
		...
		cfg.TgBot.SendToMe(tgbotapi.NewMessage(cfg.TgBot.ChatId, fmt.Sprintf("daily english error, %v", err)))
		return
	}
	fmtRes, err := ParseResponse(res)
	if err != nil {
		glog.Error("parse daily english resp error:", err)
		cfg.TgBot.SendToMe(tgbotapi.NewMessage(cfg.TgBot.ChatId, fmt.Sprintf("parse daily english resp error, %v", err)))
		return
	}
	data := make(map[string]interface{})
	data["caption"] = fmt.Sprintf("%v\n%v", fmtRes["content"], fmtRes["note"])
	data["photo"] = fmtRes["fenxiang_img"]
	data["chat_id"] = cfg.TgBot.ChatId

	url := cfg.TgBot.TgApiUrl + "/sendPhoto"
	jsonData, _ := json.Marshal(data)
	req, err := http.NewRequest("POST", url, bytes.NewBuffer(jsonData))
	req.Header.Set("Content-Type", "application/json")
	client := cfg.TgBot.Client
